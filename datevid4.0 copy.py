...
data_file_folder = './tiempos/'+mes ## Folder en donde se van a encontrar los nuevos datos ya descomprimidos
        ...
        for x in casas:
            ...
            for y in intervalos:
                ...
                if opc in file:
                    ...
                    print('Hecho: '+opc)
                    print(x)
                    qry = "LOAD DATA INFILE './tiempos/"+mes+"/"+opc+"' INTO TABLE "'houseteh'+n+'_'+y+" FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n'" ## Query para subir datos a la bd
                    print(qry)
...
def encuentraclima():
    for file in os.listdir(folder_weather): ## Encuentra el archivo en la bd
     if file.startswith('ambient'):
        ...
        print('clima')
        qry="LOAD DATA INFILE './clima/ambient2-"+mes+".csv' INTO TABLE ambient_weather FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n'"
        print(qry)
        cursor.execute(qry) ## ejecuta query 
        con.commit()
...
def descomprimir(): ## En esta funcion se descomprime el primer zip de fecha y el de cada casa2
    ...
    with zipfile.ZipFile('./'+mes+'.zip', 'r') as zip_ref: ## descomprime el zip inicial
        zip_ref.extractall('./tiempos')
    for file in os.listdir('./tiempos/'+mes):

        if file.endswith('.zip'): 
            os.chdir('./tiempos/'+mes)
            file_name = os.path.abspath('./tiempos/'+mes + "/"+ file) ## descomprime el zip de cada casa
...
def mueveclima(): ## mueve ambient-weather a la carpeta de clima y elimina el archivo para poder operar los de las casas bien.
    for file in os.listdir(data_file_folder):
            if file.startswith('ambient'):
                ...
                print('hecho ambient')
                shutil.move('./tiempos/'+mes+'/ambient-'+mes+'.csv','./clima' )
                if file.startswith('ambient'):
                    os.remove(file)
